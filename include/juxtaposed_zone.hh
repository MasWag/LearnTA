...
namespace learnta {
  ...
  public:
    ...
    JuxtaposedZone(const Zone &left, const Zone &right) : leftSize(left.getNumOfVar()), rightSize(right.getNumOfVar()) {
      ...
      this->value.block(left.value.cols(), left.value.rows(), right.value.cols() - 1,
                        right.value.rows() - 1) = right.value.block(1, 1, right.value.cols() - 1,
                                                                    right.value.rows() - 1);
      this->value.block(0, left.value.rows(), 1, right.value.rows() - 1) = right.value.block(0, 1, 1,
                                                                                             right.value.rows() - 1);
      this->value.block(left.value.cols(), 1, right.value.cols() - 1, 0) = right.value.block(1, 0,
                                                                                             right.value.cols() - 1, 1);
    }

    /*!
     * @brief Juxtapose two zones with common variables
     *
     * Let \f$x_1, x_2, \dots, x_N\f$ be the variables in left and \f$y_1, y_2, \dots, y_M\f$ be the variables in right.
    ...
    JuxtaposedZone(const Zone &left, const Zone &right, std::size_t commonVariableSize) :
      ...
      const auto rightBeginIndex = N + 1;
